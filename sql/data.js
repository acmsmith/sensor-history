...
function getData(db, sensortype, fn) {
    db.all(`select c.name, c.rowid as id, c.attribute, d.value, d.timestamp from sensor_data d
    inner join sensor_config c on d.config = c.rowid where c.attribute = ? order by c.name asc`, sensortype, 
    (err, rows) => {
      fn(rows);
    });
  }
